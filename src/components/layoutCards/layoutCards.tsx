  ...
  return (
    <div className={style.container_grid}>
      <div className={style.container_layout}>
        {pokemos.results
          .slice(AppContext.items, AppContext.items + 12)
          .map((item) => (
      ...
      <div className={style.container_pagination}>
        <p>
          <button
            type="button"
            onClick={() => {
              if (AppContext.items - 12 >= 0)
                AppContext.setItems(AppContext.items - 12);
            }}
          >
            <Image
              className={AppContext.items - 12 < 0 ? style.disable : ``}
          ...
          <button
            type="button"
            onClick={() => {
              if (AppContext.items + 12 <= pokemos.results.length)
                AppContext.setItems(AppContext.items + 12);
            }}
          >
            <Image
              className={
                AppContext.items + 12 > pokemos.results.length
                  ? style.disable
                  : ``
              }
